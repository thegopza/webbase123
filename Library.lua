...
local function should_regame()
    return _G.ReGame == true or _G.ReGameEnabled == true
end

local function get_regame_place_id()
    local pid =
        _G.ReGamePlaceId or _G.ReGamePlaceID
        or _G.PlaceId or _G.PlaceID
        or _G.placeId or _G.placeID
    return tonumber(pid)
end

local function get_regame_vip_link()
    local link =
        _G.ReGameVipLink or _G.ReGameVIPLink
        or _G.VipLink or _G.VIPLink
        or _G.PrivateServerLink or _G.VipServerLink or _G.VIPServerLink
    if link == nil then return nil end
    link = tostring(link)
    if link == "" or link == "0" then return nil end
    return link
end

local function url_decode(s)
    local ok, decoded = pcall(function()
        return http_service:UrlDecode(s)
    end)
    return ok and decoded or s
end

local function parse_private_server_code_from_link(link)
    if type(link) ~= "string" then return nil end
    local raw = link:match("privateServerLinkCode=([^&]+)")
    if not raw then return nil end
    return url_decode(raw)
end

local function parse_place_id_from_link(link)
    if type(link) ~= "string" then return nil end
    local pid = link:match("/games/(%d+)/")
    return tonumber(pid)
end

local function get_regame_job_id()
    local jid =
        _G.ReGameJobId or _G.ReGameJobID
        or _G.JobId or _G.JobID
        or _G.jobId or _G.jobID
    if jid == nil then return nil end
    jid = tostring(jid)
    if jid == "" or jid == "0" then return nil end
    return jid
end

local function get_regame_private_code()
    local code =
        _G.ReGamePrivateServerCode
        or _G.PrivateServerCode
        or _G.PrivateServerAccessCode
    if code == nil then return nil end
    code = tostring(code)
    if code == "" or code == "0" then return nil end

    if code:find("privateServerLinkCode=") then
        return parse_private_server_code_from_link(code) or code
    end

    return code
end

local function regame_teleport()
    local vip_link = get_regame_vip_link()
    local place_id = get_regame_place_id() or parse_place_id_from_link(vip_link)
    if not place_id then
        warn("re-game enabled but missing PlaceId/ReGamePlaceId; falling back to lobby")
        return false
    end

    local delay = tonumber(_G.ReGameDelay) or 0
    if delay > 0 then task.wait(delay) end

    local method = tostring(_G.ReGameMethod or "placeinstance"):lower()
    local private_code = get_regame_private_code() or parse_private_server_code_from_link(vip_link)
    local job_id = get_regame_job_id()

    local ok, err = pcall(function()
        if private_code or method == "private" or method == "privateserver" then
            if not private_code then
                error("missing private server access code (set _G.ReGamePrivateServerCode)")
            end
            teleport_service:TeleportToPrivateServer(place_id, private_code, { local_player })
        elseif job_id and (method == "placeinstance" or method == "jobid" or method == "instance") then
            teleport_service:TeleportToPlaceInstance(place_id, job_id, local_player)
        else
            teleport_service:Teleport(place_id, local_player)
        end
    end)

    if not ok then
        warn("re-game teleport failed: " .. tostring(err))
        return false
    end
    return true
end
...
local function handle_post_match()
    ...
    if not ui_root then
        if should_regame() and regame_teleport() then return end
        return send_to_lobby()
    end
